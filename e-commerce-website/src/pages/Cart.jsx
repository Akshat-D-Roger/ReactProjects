          ...
          {allItems.map((item, index) => {
            return (
              <div key={index} className="flex flex-row py-4 border-y justify-between items-center">
                ...
                <div>
                  <input type="number" value={item.quantity} min={1} step={1} onChange={(e) => { itemQuantityHandler(e, item.id, item.size, cartItems, setCartItems) }} className="w-10 border outline-none" />
                  <ToastContainer />
                </div>
                <div>
                  <button onClick={() => { deleteProduct(item, cartItems, setCartItems) }}>
